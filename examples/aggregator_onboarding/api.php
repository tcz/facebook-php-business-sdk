...
function create_client_business($access_token) {
  ...
  $child_business = (new Business($aggregator_bm_id))->getOwnedBusinesses(array(), array(
   ...
   */
  $child_business = (new Business($aggregator_bm_id))->createOwnedBusiness(array(), array(
    'id' => $aggregator_bm_id,
   ...
   */
  $api = setAccessToken($aggregator_access_token);
  (new ExtendedCredit($bm_loc_id))->createOwningCreditAllocationConfig(array(), array(
   ...
   */
  $api = setAccessToken($aggregator_access_token);
  $child_business_token = (new Business($child_business_id))->createAccessToken(array(), array(
   ...
   */
  $api = setAccessToken($child_bm_system_user_access_token);
  $account = (new Business($child_business_id))->createAdAccount(array(), array(
    'name' => 'MyAdAccount',
    'currency' => 'USD',
    'timezone_id' => 1,
...
function create_click_to_messenger_ad($account_id, $bmid, $ad_message, $page_welcome_message)
{
    global $aggregator_access_token, $app_id, $page_id;
    $api = setAccessToken($aggregator_access_token);
    $child_business_token = (new Business($bmid))->createAccessToken(array(), array(
      'app_id' => $app_id,
      'scope' => 'manage_pages,ads_management,business_management',
    ));
    $child_bm_system_user_access_token = $child_business_token->access_token;
    ...
    $campaign = $account->createCampaign(array(), array(
    ...
    $creative = $account->createAdCreative(array(), array(
      AdCreativeFields::NAME => 'Sample Creative',
      AdCreativeFields::OBJECT_STORY_SPEC => $object_story_spec,
    ));
    $ad = $account->createAd(array(), array(
      'status' => 'ACTIVE',
      'adset_id' => $ad_set_id,
      'name' => 'My Ad',
  ...
  switch ($_GET["action"]) {
    ...
    case "create_click_to_messenger_ad":
      if (
        ...
        $value = create_click_to_messenger_ad($_GET["adaccount"], $_GET["bmid"], $_GET["ad_message"], $_GET["page_welcome_message"]);
      else
        $value = "Missing argument";
      break;
  }
